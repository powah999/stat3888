...
filtered_bsp <- df_list$inp13bsp.csv %>%
  dplyr::select(ABSGID, ABSHID, SMOKQ2AE, ALCPER1, SUGPER1, HEDATTC, DIABQ01A, SABDYMS, AGEEC)
colnames(filtered_bsp) <- c("PersonID", "HouseholdID", "Cigarettes", "Alcohol", "Sugar", "Education", "Family_Diabetes", "Pregnant", "Age")
...
filtered_bhh <- df_list$inp13bhh.csv %>%
  dplyr::select(ABSHID, RAECURF)
colnames(filtered_bhh) <- c("HouseholdID", "Remoteness")
