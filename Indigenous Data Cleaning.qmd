...
# Filter and rename columns of csv data
filtered_bsp <- df_list$inp13bsp.csv %>%
  dplyr::select(ABSGID, ABSHID, SMOKQ2AE, ALCPER1, SUGPER1, HEDATTC, DIABQ01A, SABDYMS, AGEEC, SATPER1)
colnames(filtered_bsp) <- c("PersonID", "HouseholdID", "Cigarettes", "Alcohol", "Sugar", "Education", "Family_Diabetes", "Pregnant", "Age", "Fat")

filtered_bbi <- df_list$inp13bbi.csv %>%
  dplyr::select(ABSGID, ABSHID, DIABPRVE)
colnames(filtered_bbi) <- c("PersonID", "HouseholdID", "Diabetes")

filtered_bhh <- df_list$inp13bhh.csv %>%
  dplyr::select(ABSHID, RAECURF, SF2SA1DB)
colnames(filtered_bhh) <- c("HouseholdID", "Remoteness", "Socio-Economic")

# Join data on Household ID and Person ID
joined_df <- left_join(filtered_bbi, filtered_bhh, by = "HouseholdID")
joined_df <- left_join(joined_df, filtered_bsp, by = c("PersonID", "HouseholdID"))

# Filter out for our sample constraints
filtered_joined <- joined_df %>%
  filter(Pregnant != 4,
         Diabetes != 0,
         Diabetes != 8, 
         Family_Diabetes == 5,
...
filtered_joined$Cigarettes[filtered_joined$Cigarettes == 97] <- 0

filtered_joined$Age[filtered_joined$Age == 18]

# View
View(filtered_joined)

```

```{r}
ggplot(filtered_joined, aes(x = Sugar)) +
  geom_boxplot() +
  theme_minimal()
