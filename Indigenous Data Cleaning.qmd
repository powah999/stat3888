...
filtered_bbi <- df_list$inp13bbi.csv %>%
  dplyr::select(ABSGID, ABSHID, DIABPRVE, DIABFPGE)
colnames(filtered_bbi) <- c("PersonID", "HouseholdID", "Plasma_Glucose", "HBA1C")
...
filtered_joined <- joined_df %>%
  filter(Pregnant != 4,
         Plasma_Glucose != 0,
         HBA1C != 0, 
         Family_Diabetes != c(5, 6))

table(filtered_joined$Remoteness)
